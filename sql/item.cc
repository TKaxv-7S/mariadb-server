  ...
  */
  ...
  else if (ref != not_found_item)
  {
    ...
    /*
      ...
      item field occurs or on type of the query. If it is in the the having
      clause we use a wrapper of the Item_ref type. Otherwise we use a wrapper
      either of Item_direct_ref type or of Item_outer_ref. The latter is used
      for grouping queries. Such a wrapper is always added to the list
      inner_refs_list for the select against which the outer reference has been
      resolved.
    */
    ...
    *ref= save;
    if (rf)
    {
      ...
      /*
        Reverse this change if we are running a prepare statement.
        Revert any change back to the object allocated during the
          first execution if we are running any subsequent execution
      */
      if (is_first_execution)
        *reference= rf;
      else
...
bool Item_ref::fix_fields(THD *thd, Item **reference)
{
  ...
  */
  if ((*ref)->fix_fields_if_needed(thd, ref))
    goto error;
