			  ...
			  select_result_interceptor *result)
{
  ...
  */
  ...
  if (unit->item)
  {
    ...
    Item_in_subselect *in_subs= (Item_in_subselect*) (unit->item);
    if ((substype == EXISTS_SUBS &&
        ((Item_exists_subselect *)unit->item)->exists_transformed) ||
        ((substype == ALL_SUBS || substype == ANY_SUBS) &&
         (in_subs->test_set_strategy(SUBS_MAXMIN_INJECTED) ||
          in_subs->test_set_strategy(SUBS_MAXMIN_ENGINE))))
    {
      unit->item= this;
...
/*
  Rough upper bound estimate of the total number of equalities added by
  the all exists_to_in transformations applied to a unit
*/

uint st_select_lex_unit::upper_bound_of_exists2in_equalities()
{
  ...
  for (st_select_lex_unit *unit= sl->first_inner_unit();
       ...
       unit= unit->next_unit())
  {
    n+= unit->upper_bound_of_exists2in_equalities();
  }
  return n;
}
...
bool Item_exists_subselect::fix_fields(THD *thd, Item **ref)
{
  ...
  sel->select_n_reserved= unit->upper_bound_of_exists2in_equalities();
