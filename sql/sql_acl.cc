...
{
  DBUG_ASSERT(s1.str != NULL || s1.length == 0);
  DBUG_ASSERT(s2.str != NULL || s2.length == 0);
  if (!s1.str || !s2.str)
...
/**
  Helper  function for matching DENY entries.

  Lexicographically compares deny privilege entries (represented by the parts)
  The order is determined by the following fields in this order:
  (type, db, tbl, col).

  Comparison of type is numeric.
  Comparison for db, tbl parts is according to the case sensitivity rule
  (lowercase_table_names)
  Comparison for col is always case insensitive.

  @param[in] p1 - priv type of entry 1
  @param[in] d1 - db name of entry 1
  @param[in] t1 - table name of entry 1
  @param[in] c1 - column name of entry 1

  @param[in] p2 - type of entry 2
  @param[in] d2 - db name of entry 2
  @param[in] t2 - table name of entry 2
  @param[in] c2 - column name of entry 2
  @return <0 if entry 1 is smaller, 0 if they are equal, >0 if entry 2 is
  smaller.
*/
...
static bool deny_matches(ACL_PRIV_TYPE p1, const char *d1, const char *t1, const char *c1,
...
static LEX_CSTRING table_or_routine_term_by_priv_type(ACL_PRIV_TYPE type)
{
...
/*
   Maximum number of bytes needed for a JSON entry.

   Used in StringBuffer reservation when building JSON representation of DENY
   entries (deliberately chosen so as to avoid dynamic resizing of the buffer,
   and error handling complexity in edge cases)
*/
static constexpr size_t DENY_JSON_ENTRY_BUFSIZE=
 sizeof("{\"type\":\"\"column\",\"db\":\"\",\"table\":\"\",\"column\":\"\",\"bits\":}")
    + /* db, table, column, all json escaped */ (3 * NAME_LEN) * 2
    + 20 /* max digits for bits*/;
...
/**
  ...
  @note json_buf has maximum capacity of DENY_JSON_ENTRY_BUFSIZE, so this
  function will not fail due to resizing issues.
*/
static int deny_entry_to_json(const deny_data_t &entry,
                              StringBuffer<DENY_JSON_ENTRY_BUFSIZE> &json_buf)
{
  json_buf.length(0);
  ...
  json_buf.append('}');
  DBUG_ASSERT(json_buf.length() <= DENY_JSON_ENTRY_BUFSIZE);
  return 0;
}
                                ...
                                char *column_buf, size_t column_buf_len)
{
                         ...
                         &field_val, &field_len) == JSV_STRING)
  {
    ...
    if (len >= 0 && (size_t)len < table_buf_len)
    {
      ...
      return 1; // Error unescaping table/routine name
  }
  else
    return 1; // table/routine field is mandatory for non-global, non-db entry
...
class User_table_tabular: public User_table
{
 public:
  ...
  {
    my_error(ER_NOT_SUPPORTED_YET, MYF(0), "DENY with old mysql.user table");
    return 1;
  }
...
class User_table_json: public User_table
{
  ...
  void set_access(const privilege_t rights, bool revoke, bool is_deny) const override
  {
    ...
    else if (rights)
    {
      ...
      /*
       BUG: the result of update_deny_entry() is ignored.
       The probability of the error is extremely low, but it can happen, that
       tiny memory allocations in json handling fail.But it error is ignored
       to match surrounding code (set_int_value() errors are ignored too),
      */
                        ...
                        bool revoke, privilege_t &out_privs) const override
  {
    ...
    for (deny_data_t e : deny_entries())
    {
      ...
      if (json.length() > 1 && json.append(','))
        return 1;
      first_entry= false;
      StringBuffer<DENY_JSON_ENTRY_BUFSIZE> js(system_charset_info);
      if (deny_entry_to_json(e, js))
        return 1;
      if (json.append(js.ptr(), js.length()))
        return 1;
    }
    ...
    if (!found)
    {
      ...
      StringBuffer<DENY_JSON_ENTRY_BUFSIZE> entry_json(system_charset_info);
      if (deny_entry_to_json(new_entry, entry_json))
        return 1;
      if (json.append(entry_json.ptr(), entry_json.length()))
        return 1;
    }
    if (json.append(']'))
      return 1;
                                    ...
                                    const char *column, privilege_t& out_denies)
{
  ...
  /* Change entry now*/
  if (revoke_all)
    error= user_table.remove_all_deny_entries();
  else
    error= user_table.update_deny_entry(type, db, table_or_routine, column,
  ...
  if (revoke_grant)
  {
    /*
      ...
      for REVOKE.
    */
    error= clear_all_denies(combo);
    if (!error)
     error= apply_all_denies_to_caches(user_table, combo);
  }
  else
  {
    error= apply_deny_to_caches(combo, access_t(NO_ACL, out_denies), type, db,
                        table_or_routine, column);
  }
  if (error)
  {
    my_error(ER_INTERNAL_ERROR, MYF(0), "Failed to update DENY privileges in cache");
    DBUG_RETURN(-1);
  }
  DBUG_RETURN(0);
...
/* Helper for case-insensitive matching of host name */
static bool host_matches(const LEX_USER &combo, const char *host)
{
  return Lex_ident_host(combo.host).streq(
          Lex_cstring_strlen(safe_str(host)));
}
...
static int apply_deny_db(const LEX_USER &combo, const access_t& acc, const char *db)
{
  ...
  if (apply_deny_user(combo, parent_acc))
    return 1;
  return 0;
}
...
static int apply_deny_table(const LEX_USER &combo,const access_t& acc, const char *db, const char *table)
{
  ...
  access_t parent_acc(NO_ACL, NO_ACL, acc.deny_bits()|acc.deny_subtree());
  if (apply_deny_db(combo, parent_acc, db))
    return 1;
  return 0;
}
...
static int apply_deny_column(const LEX_USER &combo, const access_t &access,const char *db, const char *table, const char *column)
{
  ...
  */
  ...
  access_t parent_acc(NO_ACL, NO_ACL, access.deny_bits()|access.deny_subtree());
  if (apply_deny_table(combo, parent_acc, db, table))
    return 1;
  return 0;
}
  ...
  */
  ...
  for (tl= tables; number-- ; tl= tl->next_global)
  {
    ...
    */
    ...
    denied= t_ref->grant.privilege.denied_privs(original_want_access);
    if (denied)
    {
      want_access= denied;
      goto err;
                        ...
                        Security_context *sctx)
{
  ...
  want_access= grant->privilege.denied_privs(grant->want_privilege);
  if (want_access)
    goto err;
  ...
  */
  ...
  for (; !fields->end_of_fields(); fields->next())
  {
    ...
    if (table_name != fields->get_table_name().str)
    {
      ...
      denied= grant->privilege.denied_privs(want_access_arg);
      if (denied)
      {
        want_access= denied;
        goto err;
    ...
    if (want_access || has_col_deny)
    {
      ...
      privilege_t col_denied= have_access.denied_privs(want_access_arg);
      if (col_denied)
      {
...
bool check_grant_db(THD *thd, const access_t &access, const char *db)
{
  ...
  if (access.denied_privs(TABLE_ACLS | PROC_ACLS) == (TABLE_ACLS | PROC_ACLS))
