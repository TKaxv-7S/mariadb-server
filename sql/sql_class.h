...
public:
  ...
  */
  ...
  /* Check that binlog_state variable is correct */
#ifndef DBUG_OFF
  void check_binlog_state();
#else
  inline void check_binlog_state()
  {}
#endif
  inline void set_binlog_bit()
  {
    if (variables.sql_log_bin)
    {
      ...
      /* Reset BINLOG_STATE_BYPASS if no disabled states */
  ...
  /*
    ...
    Note that if Galera is used without a binary log, some flags are
    ignored to ensure that all changes are sent to the other nodes
  ...
  */
  ...
  {
    return ((binlog_state &
             (unlikely(variables.wsrep_on && WSREP_PROVIDER_EXISTS_) ?
   ...
   /* WSREP as above, but no check if Galara is using wsrep_emulate_bin_log */
  ...
  /*
    ...
    It is not exact as some things may have changed since last query,
    but can be used as a precheck to see if there is chance that binary
    logging could happen.
    In essence BINLOG_STATE_FILTER, BINLOG_STATE_READONLY and
    BINLOG_STATE_WSREP_DISABLED are not checked
  */
  ...
  /*
    ...
    (where replication is disabled because of statement based logging).
    It is used to decide things like 'will auto increment usage cause
    binlogging'.
    See handler::update_auto_increment().
    We have to ignore states
    BINLOG_STATE_TMP_DISABLED      Used by sp_head::execute_function()
    BINLOG_STATE_SUBSTMT_DISABLED  Used by THD::reset_sub_statement_stat()
    BINLOG_STATE_UNION_DISABLED    Used by start_union_events()
    It is ok to ignore BINLOG_STATE_BYPASS as if binlogging is disabled there is
    always another bit set.
  */
    ...
    return ((binlog_state &
             ((BINLOG_STATE_DISABLED &
               ~(BINLOG_STATE_TMP_DISABLED | BINLOG_STATE_SUBSTMT_DISABLED |
                 BINLOG_STATE_UNION_DISABLED | BINLOG_STATE_BYPASS)) |
              BINLOG_STATE_ACTIVE_WSREP)) ==
            BINLOG_STATE_ACTIVE_WSREP);
  }
  inline void enable_wsrep_on()
  {
#ifdef WITH_WSREP
  ...
  inline void set_wsrep_on(int set)
  {
    if (set)
      enable_wsrep_on();
    else
