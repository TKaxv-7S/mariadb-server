...
/*
  One non-driving join table as run by a worker: its private TABLE copy, the
  access method, the cloned (worker-bound) ref for index access, and the cloned
  per-table condition. Defined in sql_parallel_workers.cc (needs TABLE_REF).
*/
...
public:
  ...
  */
  ...
  void close_worker_tables(pwt_worker *worker);
  void free_queue();
