...
static void dbug_add_print_item_contents(Item *item)
{
  ...
  if (item->type() == Item::REF_ITEM)
  {
    ...
    if (ref)
    {
      ...
      {
        DBUG_SPRINTF_CAT("(Item_ref*)%p(name:'%s.%s',", item,
          ((Item_ref*)item)->table_name.str, item->name.str);
      }
      else
        DBUG_SPRINTF_CAT("(Item_ref*)%p(name:'%s',", item, item->name.str);
      ...
      DBUG_SPRINTF_CAT("(Item_ref*)nullptr");
  }
  else
  {
...
static void dbug_add_print_items(List<Item> &list)
{
  ...
  while ((sub_item= li++))
    cnt++;
  DBUG_SPRINTF_CAT("Children[%d]", cnt);
  do_obj_indent();
  li.rewind();
  while ((sub_item= li++))
  {
    // TODO: significance of magic number 7
...
{
  *dbug_big_buffer= (char)0;
  if (args->argument_count())
  {
    DBUG_CAT("Arguments:");
    for (uint i= 0; i < args->argument_count(); i++)
    {
      Item *arg= args->arguments()[i];
      DBUG_SPRINTF_CAT("[%p: ", (void*)arg);
      dbug_add_print_item(arg);
      DBUG_CAT("]");
    }
  }
  Item *item= static_cast<Item*>(args);
  dbug_add_print_item((Item *)args);
...
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("List<Item_outer_ref> %p", (void*) &list);
...
static void dbug_add_print_item(Item *item)
{
  ...
  if (item)
  {
    ...
    case Item::REF_ITEM:
      {
        ...
        if (ref)
        {
              ...
              ((Item_ref*)item)->table_name.str)
          {
            ...
            DBUG_SPRINTF_CAT("(Item_ref*)(name:'%s',", item->name.str);
          if (ref == not_found_item)
            DBUG_CAT("not_found_item");
          else
          {
    ...
    case Item::FUNC_ITEM:
      {
        ...
        case Item_func::MULT_EQUAL_FUNC:
          {
          DBUG_SPRINTF_CAT("(Item_equal*)%p", item);
          Item_equal *item_equal= (Item_equal *)func_item;
          Item_equal_fields_iterator it(*item_equal);
          Item *itm= item_equal->get_const();
          if (itm)
        ...
        case Item_func::EQ_FUNC: DBUG_SPRINTF_CAT("(Item_func_eq*)%p", item); break;
        case Item_func::LIKE_FUNC: DBUG_SPRINTF_CAT("(Item_func_like*)%p", item); break;
        case Item_func::EQUAL_FUNC: DBUG_SPRINTF_CAT("(Item_func_equal*)%p", item); break;
        case Item_func::ISNULL_FUNC: DBUG_SPRINTF_CAT("(Item_func_isnull*)%p", item); break;
        case Item_func::UNKNOWN_FUNC:
          if (func_item->func_name())
            DBUG_SPRINTF_CAT("(%s*)%p", func_item->func_name(), item);
          else
            DBUG_SPRINTF_CAT("(Item_func*)%p", item);
          break;
        case Item_func::NE_FUNC: DBUG_SPRINTF_CAT("(Item_func_ne*)%p", item); break;
        case Item_func::LT_FUNC: DBUG_SPRINTF_CAT("(Item_func_lt*)%p", item); break;
        ...
        case Item_func::XOR_FUNC: DBUG_SPRINTF_CAT("(Item_func_xor*)%p", item); break;
        case Item_func::BETWEEN: DBUG_SPRINTF_CAT("(Item_func_between*)%p", item); break;
        ...
        case Item_func::TEMPTABLE_ROWID: DBUG_SPRINTF_CAT("(Item_temptable_rowid*)%p", item); break;
        case Item_func::NOW_FUNC: DBUG_SPRINTF_CAT("(Item_func_now_local*)%p", item); break;
        case Item_func::NOW_UTC_FUNC: DBUG_SPRINTF_CAT("(Item_func_now_utc*)%p", item); break;
        case Item_func::SYSDATE_FUNC: DBUG_SPRINTF_CAT("(Item_func_sysdate_local*)%p", item); break;
        case Item_func::TRIG_COND_FUNC: DBUG_SPRINTF_CAT("(Item_func_trig_cond*)%p", item); break;
        case Item_func::SUSERVAR_FUNC: DBUG_SPRINTF_CAT("(Item_func_set_user_var*)%p", item); break;
        case Item_func::GUSERVAR_FUNC: DBUG_SPRINTF_CAT("(Item_func_get_user_var*)%p", item); break;
        case Item_func::COLLATE_FUNC: DBUG_SPRINTF_CAT("(Item_func_set_collation*)%p", item); break;
        case Item_func::EXTRACT_FUNC: DBUG_SPRINTF_CAT("(Item_extract*)%p", item); break;
        case Item_func::CHAR_TYPECAST_FUNC: DBUG_SPRINTF_CAT("(Item_char_typecast*)%p", item); break;
        case Item_func::FUNC_SP: DBUG_SPRINTF_CAT("(Item_func_sp*)%p", item); break;
        case Item_func::UDF_FUNC: DBUG_SPRINTF_CAT("(Item_udf_func*)%p", item); break;
        case Item_func::NEG_FUNC: DBUG_SPRINTF_CAT("(Item_func_neg*)%p", item); break;
        case Item_func::GSYSVAR_FUNC: DBUG_SPRINTF_CAT("(Item_func_get_system_var*)%p", item); break;
        case Item_func::IN_OPTIMIZER_FUNC: DBUG_SPRINTF_CAT("(Item_in_optimizer*)%p", item); break;
        case Item_func::DYNCOL_FUNC: DBUG_SPRINTF_CAT("(Item_func_dyncol_create*)%p", item); break;
        case Item_func::JSON_EXTRACT_FUNC: DBUG_SPRINTF_CAT("(Item_func_json_extract*)%p", item); break;
        case Item_func::CASE_SEARCHED_FUNC: DBUG_SPRINTF_CAT("(Item_func_case_searched*)%p", item); break;
        case Item_func::CASE_SIMPLE_FUNC: DBUG_SPRINTF_CAT("(Item_func_case_simple*)%p", item); break;
        default:
          DBUG_SPRINTF_CAT("(ITEM_FUNC_OTHER*)%p", item);
        }

        for (uint i= 0; i < func_item->argument_count(); i++)
          dbug_add_print_item(func_item->arguments()[i]);
      }
      break;
    case Item::COND_ITEM:
      {
        ...
        case Item_func::COND_OR_FUNC: DBUG_SPRINTF_CAT("(Item_cond_or*)%p", item); break;
        default:
          DBUG_SPRINTF_CAT("(Item_cond*)%p", item);
        }
        List_iterator<Item> li= *((Item_cond*) item)->argument_list();
        Item *sub_item;
        while ((sub_item= li++))
        {
          if (strlen(dbug_big_buffer) < DBUG_BIG_BUFFER_SIZE-
                                        DBUG_ITEM_BUFFER_SIZE-7)
            dbug_add_print_item(sub_item);
          else
          {
    ...
    case Item::FIELD_ITEM:
      DBUG_SPRINTF_CAT("(Item_field*)%p", item);
      dbug_add_print_item_contents(item);
      break;
#if MYSQL_VERSION_ID > 100400
    case Item::CONST_ITEM:
      DBUG_SPRINTF_CAT("(Item_literal*)%p", item);
      dbug_add_print_item_contents(item);
      break;
#endif
    case Item::SUM_FUNC_ITEM:
      DBUG_SPRINTF_CAT("(Item_sum*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::WINDOW_FUNC_ITEM:
      DBUG_SPRINTF_CAT("(Item_window_func*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::NULL_ITEM:
      DBUG_SPRINTF_CAT("(Item_null*)%p", item);
      break;
    case Item::COPY_STR_ITEM:
      DBUG_SPRINTF_CAT("(Item_copy*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::FIELD_AVG_ITEM:
      DBUG_SPRINTF_CAT("(Item_avg_field*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::DEFAULT_VALUE_ITEM:
      DBUG_SPRINTF_CAT("(Item_default_value*)%p", item);
      dbug_add_print_item_contents(item);
      break;
#if MYSQL_VERSION_ID > 100414
    case Item::CONTEXTUALLY_TYPED_VALUE_ITEM:
      DBUG_SPRINTF_CAT("(Item_contextually_typed_value_specification*)%p", item);
      dbug_add_print_item_contents(item);
      break;
#endif
    case Item::PROC_ITEM:
      DBUG_SPRINTF_CAT("(Item_proc*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::FIELD_STD_ITEM:
      DBUG_SPRINTF_CAT("(Item_std_field*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::FIELD_VARIANCE_ITEM:
      DBUG_SPRINTF_CAT("(Item_variance_field*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::INSERT_VALUE_ITEM:
      DBUG_SPRINTF_CAT("(Item_insert_value*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::ROW_ITEM:
      DBUG_SPRINTF_CAT("(Item_row*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::CACHE_ITEM:
      DBUG_SPRINTF_CAT("(Item_cache*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::TYPE_HOLDER:
      DBUG_SPRINTF_CAT("(Item_type_holder*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::PARAM_ITEM:
      DBUG_SPRINTF_CAT("(Item_param*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::TRIGGER_FIELD_ITEM:
      DBUG_SPRINTF_CAT("(Item_trigger_field*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::EXPR_CACHE_ITEM:
      DBUG_SPRINTF_CAT("(Item_cache_wrapper*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    case Item::SUBSELECT_ITEM:
      DBUG_SPRINTF_CAT("(Item_subselect*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    default:
      DBUG_SPRINTF_CAT("(ITEM_UNKNOWN*)%p", item);
      dbug_add_print_item_contents(item);
      break;
    }
  }
...
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(Ref_ptr_array*)%p\n", (void*) ptr_array);
  Item *item;
...
const char *dbug_print_select_lex(st_select_lex *sl)
{
  ...
  *dbug_item_small_buffer= (char)0;
  indent_level= 0;
  DBUG_SPRINTF_CAT("(st_select_lex*)%p", sl);
  if (sl)
  {
...
const char *DBUG_PRINT_FUNCTION(KEY_FIELD *arg)
{
  big_buffer_start();
  DBUG_CAT("(KEY_FIELD*){field;");
  dbug_add_print_field(arg->field);
  DBUG_CAT(",cond:");
  dbug_add_print_item(arg->cond);
  DBUG_CAT(",val:");
  dbug_add_print_item(arg->val);
  DBUG_CAT("}");
  return dbug_big_buffer;
}
...
{
  DBUG_SPRINTF_CAT("(Item_subselect::Ref_to_outside*)%p\n", ref);
  if (ref)
  {
    if (ref->item)
...
const char *DBUG_PRINT_FUNCTION(Longlong *ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(Longlong) %ll", ref->value());
  return dbug_big_buffer;
}
const char *DBUG_PRINT_FUNCTION(Longlong ref)
...
const char *DBUG_PRINT_FUNCTION(ULonglong *ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(Longlong) %ul", ref->value());
  return dbug_big_buffer;
}
const char *DBUG_PRINT_FUNCTION(ULonglong ref)
  {return DBUG_PRINT_FUNCTION(&ref);}
...
const char *DBUG_PRINT_FUNCTION(String *ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(String) \"%s\"", ref->ptr());
  return dbug_big_buffer;
}
const char *DBUG_PRINT_FUNCTION(String ref)
  {return DBUG_PRINT_FUNCTION(&ref);}
...
const char *DBUG_PRINT_FUNCTION(LEX_CSTRING k)
  { return DBUG_PRINT_FUNCTION(&k); }
const char *DBUG_PRINT_FUNCTION(List<Item_equal> *ref)
  {return DBUG_PRINT_FUNCTION((List<Item> *)ref);}
const char *DBUG_PRINT_FUNCTION(List<Item_equal> ref)
  {return DBUG_PRINT_FUNCTION((List<Item> *)&ref);}
const char *DBUG_PRINT_FUNCTION(int ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(int) %d", ref);
  return dbug_big_buffer;
}
const char *DBUG_PRINT_FUNCTION(long ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(long) %ld", ref);
  return dbug_big_buffer;
}
const char *DBUG_PRINT_FUNCTION(double ref)
{
  big_buffer_start();
  DBUG_SPRINTF_CAT("(double) %f", ref);
  return dbug_big_buffer;
}
