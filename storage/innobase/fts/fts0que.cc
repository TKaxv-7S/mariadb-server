...
@return DB_SUCCESS to continue processing, DB_SUCCESS_LOCKED_REC to stop, or error code */
static dberr_t node_query_processor(
   ...
   const rec_offs* offsets, void* user_arg)
{
  ...
  bool ilist_external= rec_offs_nth_extern(offsets, 6);
  mem_heap_t* ilist_heap= nullptr;
  byte* external_ilist= nullptr;

  if (ilist_external && ilist_data && ilist_len != UNIV_SQL_NULL)
  {
    ilist_heap= mem_heap_create(ilist_len + 1000);
    ulint external_len;
    external_ilist= btr_copy_externally_stored_field(
      &external_len, ilist_data, index->table->space->zip_size(),
      ilist_len, ilist_heap);
    if (external_ilist)
    {
      ilist_data= external_ilist;
      ilist_len= external_len;
    }
    else
    {
      if (ilist_heap) mem_heap_free(ilist_heap);
      return DB_SUCCESS;
    }
  }
  ...
  if (!skip)
  {
    if (ilist_data && ilist_len != UNIV_SQL_NULL && ilist_len > 0)
    {
      ...
      if (query->error != DB_FTS_EXCEED_RESULT_CACHE_LIMIT)
        err= query->error;
    }
  }

  if (ilist_heap)
    mem_heap_free(ilist_heap);
  return err;
}
	...
	const fts_string_t*	token)	/*!< in: token to search */
{
	...
	if (query->doc_ids && !rbt_empty(query->doc_ids)) {
		...
		error = fts_index_fetch_nodes(
			query->executor, query->index, token, query,
	...
	fts_string_t*		token)	/*!< in: token to search */
{
	...
	error = fts_index_fetch_nodes(
		query->executor, query->index, token, query, node_query_processor,
		compare_mode);
                                 ...
                                 bool expansion= false)
{
                                            ...
                                            const rec_offs* offsets) -> dberr_t
  {
    ...
    return phrase->found ? DB_SUCCESS_LOCKED_REC : DB_SUCCESS; /* Stop if found, continue if not found */
  };
  ...
  err= executor.read_by_index(user_table, fts_doc_id_index,
                              &search_tuple, PAGE_CUR_GE, reader, true);
	...
	THD*		thd)		/*!< in: query THD */
{
	...
	*found = phrase.found = FALSE;
	...
	error = fts_query_fetch_document(
		get_doc->index_cache->index, match->doc_id, &phrase, thd);
	...
	fts_result_t**	result)
{
	...
	*result = NULL;
	...
	/* Inherit the user trx's THD so internal FTS query trx uses the
	caller's wait-timeout policy and per-THD wait accounting. */
	query_trx->mysql_thd = trx->mysql_thd;
	...
	error = query.executor->open_all_aux_tables(index);
	if (error == DB_SUCCESS) {
	...
	if (error == DB_SUCCESS) {
		...
		error = fts_table_fetch_doc_ids(
			query.executor, index->table, "DELETED",
			query.deleted);
	}
...
func_exit:
	/* Clean up the dynamically allocated executor */
	if (query.executor) {
