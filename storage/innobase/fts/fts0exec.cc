  ...
  const rec_offs *offsets, void *user_arg)
{
  ...
  if (word_data && word_len > 0 && word_len != UNIV_SQL_NULL)
  {
    if (!m_word_heap)
      m_word_heap= mem_heap_create(256);

    if (m_last_word.f_str)
      mem_heap_empty(m_word_heap);

    m_last_word.f_str= static_cast<byte*>(
      mem_heap_dup(m_word_heap, word_data, word_len));
    m_last_word.f_len= word_len;
    m_last_word.f_n_char= 0;
  }
